...
public class GameConfigurationMapper {
  ...
  public GameConfiguration mapToGameConf(ConfigurationGameDto configurationGameDto) {
    return new GameConfiguration(configurationGameDto.getRoundTime(), configurationGameDto.getNumberOfRounds(),
        configurationGameDto.getInitialMoneyAmount(), configurationGameDto.getPoolMultiplierFactor());
  }
  ...
  public ConfigurationGameDto mapToDto(GameState gameState, Optional<GameConfiguration> gameConfiguration) {
    return gameConfiguration.map(configuration -> new ConfigurationGameDto(gameState.getSessionToken().getStrToken(),
            gameState.getGameName(), gameState.getDescription(),
            configuration.getRoundTime(), configuration.getNumberOfRounds(), configuration.getInitialMoneyAmount(),
            configuration.getPoolMultiplierFactor(), true, gameState.getStatus(),
            frontendUrl + "/user/game/" + gameState.getSessionToken().getStrToken()))
        .orElseGet(() -> new ConfigurationGameDto(gameState.getSessionToken().getStrToken(),
            gameState.getGameName(), gameState.getDescription(), -1, -1, -1, -1,
            false, gameState.getStatus(), "..."));
  }
